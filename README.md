...
1. Decide where to host it (Drive, YouTube, or repo). Get a shareable URL.
2. Open the relevant systems HTML file in GitHub (pencil icon to edit).
3. Find a `<div class="ph">Video coming</div>` line you want to fill.
4. Replace just that line with the right embed code:
...
5. Commit. Live in ~60 seconds.

## Adding a PDF

1. Upload PDF to `assets/pdf/` via GitHub web UI (Add file → Upload files).
2. In the relevant page, find the `<ul class="resources">` list and add:
   ```html
...
1. Upload the image to `assets/img/` (e.g. `blocking-zones.png`).
2. Edit the file → find the HTML comment block that wraps the diagram → delete the `<!--` and `-->` lines around it.
3. Commit.
